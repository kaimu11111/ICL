...
class MetaICLModel(object):
    ...
    def __init__(self, gpt2="gpt2-large", logger=None, 
        ...
        if soft_prefix:
            # self.model.load_state_dict(torch.load(prefix_embed_file))
    ...
    def save(self, step, save_all=True):
        if self.local_rank <= 0:
            if save_all:
    ...
    def do_train(self, data, batch_size, num_training_steps, save_period, log_period,
        ...
        #     # print(f"Step : {step} , batch is : {batch} shape is {len(batch[0][0])}")
        ...
        for epoch in range(num_training_steps):
            ...
            for batch in dataloader:
                ...
                # print("input id shape is ",[len(input_ids),len(input_ids[0])] )
                # print("attention_mask is ",attention_mask)
                # print("attention_mask shape is ",[len(attention_mask),len(attention_mask[0])] )
                # print("token_type_ids is ",token_type_ids)
                # print("token_type_ids is ",[len(token_type_ids),len(token_type_ids[0])] )
                shapeOfNonZero = []
                for i in range(len(input_ids)):
                    nonZero = 0
                    for k in range(len(input_ids[0])):
                        if input_ids[i][k] != 0:
                            nonZero += 1
                    shapeOfNonZero.append([nonZero])
                # print("nonZero length is ", shapeOfNonZero)
                if len(batch)==3:
                    labels=None
                else:
    ...
    def do_predict(self, data, batch_size=1, losses=None, verbose=False, return_nll=False):
        ...
        gt_labels = []
        pred_labels = []
        # print(f"data.metadata looks like: {data.metadata}")
        # print("-----------------------------------------------------------------")
    ...
    def run_model(self, input_ids, attention_mask, token_type_ids, labels=None):
        ...
        # print(f"outputs shape is ------------({len(outputs)},{len(outputs[0])},{len(outputs[0][0])})")
        logits = outputs.logits[..., :-1, :].contiguous()
        # print("outputs is ---------", logits)
        if labels is None:
            labels = input_ids
